...
def predict_scalable(covar_module0, likelihood, train_xt, test_x, mu, z, N, latent_dim, eps):
    """
    Helper function to perform predictions.
    ...
    x_st = torch.reshape(train_xt, [N, Q])
    stacked_x_st = torch.stack([x_st for i in range(latent_dim)], dim=0)
    mu = mu.T
    mu_st = torch.reshape(mu, [latent_dim, N, 1])

    K0xz = covar_module0(train_xt, z).evaluate()
    K0zz = covar_module0(z, z).evaluate() + eps * I_M
    K0Xz = covar_module0(test_x, z).evaluate()

    B_st = torch.eye(N, dtype=torch.double).to(device) * likelihood.noise_covar.noise.unsqueeze(dim=2).to(device)
    LB_st = torch.cholesky(B_st)
    iB_st = torch.cholesky_solve(torch.eye(N, dtype=torch.double).to(device), LB_st)

    K0xz_st = torch.reshape(K0xz, [latent_dim, N, M])
    K0zx_st = K0xz_st.transpose(-1, -2)


    iB_K0xz = torch.matmul(iB_st, K0xz_st)
    K0zx_iB_K0xz = torch.matmul(K0xz.transpose(-1, -2), torch.reshape(iB_K0xz, [latent_dim, N, M]))

    H = K0zz + K0zx_iB_K0xz

    iB_mu = torch.matmul(iB_st, mu_st).view(latent_dim, -1, 1)
    K0xz_iH_K0zx_iB_mu_st = torch.matmul(K0xz, torch.solve(torch.matmul(K0xz.transpose(-1, -2), iB_mu), H)[0]).reshape(latent_dim, N, 1)

    iB_K0xz_iH_K0zx_iB_mu = torch.matmul(iB_st, K0xz_iH_K0zx_iB_mu_st).view(latent_dim, -1, 1)

    mu_tilde = iB_mu - iB_K0xz_iH_K0zx_iB_mu

    K0Xz_iK0zz_K0zx_mu_tilde = torch.matmul(K0Xz, torch.solve(torch.matmul(K0xz.transpose(-1, -2), mu_tilde), K0zz)[0])

    Z_pred = K0Xz_iK0zz_K0zx_mu_tilde.squeeze(dim=2).T

    return Z_pred


def predict_simple(covar_module, likelihood, train_xt, test_x, z, P, T, id_covariate, eps):
    """
    Helper function to perform predictions.
    ...
    K1_st = covar_module(train_xt, train_xt).evaluate()
    B_st = K1_st + I_T * likelihood.noise_covar.noise

    LB_st = torch.cholesky(B_st)
    iB_st = torch.cholesky_solve(I_T, LB_st)

    K0Xx = covar_module(test_x, train_xt).evaluate()

    Z_pred = torch.matmul(torch.matmul(K0Xx, iB_st), z)
